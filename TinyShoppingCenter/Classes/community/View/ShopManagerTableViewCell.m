...
-(void)createTopViews:(NSString *)typeStr{
    ...
    nomalImage = [UIImage imageNamed:@"selectgrey"];
    selectedImage = [UIImage imageNamed:@"selectred"];
    ...
    [_selectedButton mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_offset(10*SCREEN_WIDTH_SCALE);
    ...
    [_iconImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(_selectedButton.mas_right).offset(10*SCREEN_WIDTH_SCALE);
    ...
    //价格
    _priceLable = [UILabel configureLabelWithTextColor:RedColor textAlignment:NSTextAlignmentLeft font:MAIN_TITLE_FONT];
...
-(void)clickCountButton:(UIButton *)button{
    ...
    [KUserdefaults setObject:_goodModel.count forKey:_goodModel.ID];
...
-(void)setModel:(GoodModel *)model{
    ...
    //商品图片
    NSString *imageString = [BaseImagestr_Url stringByAppendingString:model.url];
    [_iconImageView sd_setImageWithURL:[NSURL URLWithString:imageString] placeholderImage:[UIImage imageNamed:@"wait"]];
    // 名称
    ...
    // 从本地取出count值
    NSString *countStr = [NSString stringWithFormat:@"%@",[KUserdefaults objectForKey:_goodModel.ID]];
